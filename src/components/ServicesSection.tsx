  ...
  return (
    <div ref={container} className="h-[100vh] flex items-center justify-center sticky top-0 px-4 md:px-0">
      <motion.div 
        ...
        className="relative bg-[#FAFAFA] text-black rounded-[32px] md:rounded-[48px] p-5 md:p-10 lg:p-14 2xl:p-20 flex flex-col md:flex-row gap-8 md:gap-12 lg:gap-16 hover:bg-white transition-all duration-500 group w-full max-w-[1600px] 2xl:max-w-[1800px] shadow-[0_20px_50px_rgba(0,0,0,0.3)] border border-white/20 overflow-hidden"
      >
        <motion.div 
        ...
        >
          <div>
            ...
            <h3 className="text-3xl sm:text-4xl md:text-5xl lg:text-[54px] 2xl:text-[72px] font-normal tracking-tighter uppercase mb-4 leading-[0.85] pr-4 md:pr-12">
              {service.title.split(' ').map((word: string, i: number) => (
                <span key={i} className="block">{word}</span>
              ))}
            </h3>
            <p className="text-[12px] md:text-[14px] lg:text-[16px] 2xl:text-[18px] font-normal opacity-60 max-w-full md:max-w-[90%] leading-[1.6] mb-6 md:mb-8">
...
export default function ServicesSection({ data }: ServiceProps) {
  ...
  return (
    <section ref={container} className="bg-background relative z-10" id="services">
      <div className="max-w-[1800px] 2xl:max-w-[2000px] px-6 md:px-12 lg:px-24 mx-auto pt-16 pb-32 lg:pt-20 lg:pb-[20vh]">
